...
public class BBSCommands
{
    ...
    /**
     * /bbs morph McHorse( Ported to 1.21.1 by Over ) - demorph (remove morph) player McHorse( Ported to 1.21.1 by Over )
     */
    ...
    /**
     * /bbs morph McHorse( Ported to 1.21.1 by Over ) {id:"bbs:model",model:"butterfly",texture:"assets:models/butterfly/yellow.png"}
     *
     * Morphs player McHorse( Ported to 1.21.1 by Over ) into a butterfly model with yellow skin
     */
    ...
    /**
     * /bbs film McHorse( Ported to 1.21.1 by Over ) play test - Plays a film (with camera) to McHorse( Ported to 1.21.1 by Over )
    ...
    /**
     * /bbs film McHorse( Ported to 1.21.1 by Over ) stop test - Stops film playback
     */
    ...
    private static int saveStructure(CommandContext<ServerCommandSource> source)
    {
        ...
        try
        {
            structureTemplate = structureTemplateManager.getTemplateOrBlank(Identifier.tryParse(name));
        }
        ...
        try
        {
            if (structureTemplateManager.saveTemplate(Identifier.tryParse(name)))
            {
                return 1;
            }
        }
