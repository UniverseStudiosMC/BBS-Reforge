...
public abstract class BaseFilmController
{
    ...
    public static void renderEntity(FilmControllerContext context)
    {
        ...
        // Add dynamic light from held items (like torches)
        int heldLight = getHeldItemLuminance(entity);
        torch = Math.max(torch, heldLight);

        // Register this entity as a dynamic light source for environmental lighting
        if (heldLight > 0)
        {
            DynamicLightManager.getInstance().updateEntityLight(entity, position.x, position.y + 0.5, position.z);
        }
    ...
    /**
     * Calculate the luminance of items held by an entity (main hand and off hand).
     * This simulates dynamic lighting from held light sources like torches.
     */
    private static int getHeldItemLuminance(IEntity entity)
    {
        int maxLight = 0;
     ...
     * For block items, returns the block's light emission level.
     */
    private static int getItemLuminance(ItemStack stack)
    {
    ...
    protected void renderEntity(WorldRenderContext context, Replay replay, IEntity entity)
    {
        if (!replay.actor.get())
        {
            ...
            filmContext.transition = getTransition(entity, MinecraftClient.getInstance().getRenderTickCounter().getTickDelta(true));
