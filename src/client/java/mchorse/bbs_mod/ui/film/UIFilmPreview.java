...
public class UIFilmPreview extends UIElement
{
    ...
    private void renderCursor(UIContext context)
    {
        ...
        // In 1.21.1, RenderSystem.getModelViewStack() returns org.joml.Matrix4fStack (not MatrixStack)
        org.joml.Matrix4fStack modelView = RenderSystem.getModelViewStack();

        modelView.pushMatrix();

        // Apply the context's transformation matrix
        modelView.mul(context.batcher.getContext().getMatrices().peek().getPositionMatrix());

        // Apply our transformations
        modelView.translate(area.x + 16, area.ey() - 12, 0F);
        modelView.rotateX((float) Math.toRadians(-mcCamera.getPitch()));
        modelView.rotateY((float) Math.toRadians(mcCamera.getYaw()));
