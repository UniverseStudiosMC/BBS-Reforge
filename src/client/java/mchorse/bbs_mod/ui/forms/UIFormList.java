...
public class UIFormList extends UIElement
{
    ...
    public void setSelected(Form form)
    {
        ...
        MapType formData = form != null ? FormUtils.toData(form) : null;

        for (UIFormCategory category : this.categories)
        {
            Form matchedForm = this.findMatchingForm(category.category.getForms(), form, formData);
            ...
            else
            {
                ...
                category.select(matchedForm, false);
            }
        }
    ...
    /**
     * Find a form in the list that matches by reference or by serialized data
     */
    private Form findMatchingForm(List<Form> forms, Form form, MapType formData)
    {
        if (form == null)
        {
            return null;
        ...
        // If not found by reference, compare by serialized data
        if (formData != null)
        {
            for (Form f : forms)
            {
                MapType fData = FormUtils.toData(f);

                if (fData != null && fData.equals(formData))
                {
                    return f;
                }
            }
