...
public class BillboardFormRenderer extends FormRenderer<BillboardForm>
{
    ...
    public void render3D(FormRenderingContext context)
    {
        ...
        // MINECRAFT 1.21.1 API CHANGE - getPositionTexLightmapColorProgram() was removed, using getPositionTexColorProgram() instead
    ...
    private VertexConsumer fill(VertexFormat format, VertexConsumer consumer, Matrix4f matrix, float x, float y, Color color, float u, float v, int overlay, int light, MatrixStack.Entry entry, float nz)
    {
        ...
        // MINECRAFT 1.21.1 API CHANGE - normal() now takes just (x, y, z) and transforms using the current matrix stack entry internally
        return consumer.vertex(matrix, x, y, 0F).color(color.r, color.g, color.b, color.a).texture(u, v).overlay(overlay).light(light).normal(0F, 0F, nz);
    }
}